...
class CNNModel2D(nn.Module):
    def __init__(self, args, alphabet_size, num_cls, classifier=False):
        ...
        self.linear = nn.Conv2d(inp_size, args.hidden_dim, kernel_size=9)
        self.time_embedder = nn.Sequential(GaussianFourierProjection(embed_dim= args.hidden_dim),nn.Linear(args.hidden_dim, args.hidden_dim))

        self.num_layers = 4 * args.num_cnn_stacks
        self.convs = nn.ModuleList([layer for layer in [nn.Conv2d(args.hidden_dim, args.hidden_dim, kernel_size=9, padding=4),
                      nn.Conv2d(args.hidden_dim, args.hidden_dim, kernel_size=9, padding=4),
                      nn.Conv2d(args.hidden_dim, args.hidden_dim, kernel_size=9, dilation=2, padding=8),
                      nn.Conv2d(args.hidden_dim, args.hidden_dim, kernel_size=9, dilation=4, padding=16)
    ...
    def forward(self, seq, t, cls = None, return_embedding=False):
        if self.args.clean_data:
            ...
            feat = F.pad(feat, (4, 4, 4, 4), mode='circular')
            feat = self.linear(feat)
        else:
            time_emb = F.relu(self.time_embedder(t))
            # feat = seq.permute(0, 2, 1)
